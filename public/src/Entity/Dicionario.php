...
class Dicionario
{
    ...
    public function setData($data)
    {
        ...
        if (!empty($data)) {
            if (is_numeric($data)) {
                $read = new Read();
                $read->exeRead($this->entity, "WHERE id = :id", "id={$data}", !0);
                if ($read->getResult()) {
    ...
    private function readMultValues(Meta $m, int $id): array
    {
        $data = [];
        $read = new Read();
        $read->exeRead(PRE . $this->entity . "_" . $m->getColumn(), "WHERE {$this->entity}_id = :id", "id={$id}", !0);
        if ($read->getResult()) {
    ...
    private function createUpdateUser(string $entity, array $dados, string $action)
    {
        ...
        if (!empty($info['user']) && $info['user'] === 1) {
            ...
            if ($action === "update") {
                $read = new Read();
                $read->exeRead($entity, "WHERE id = :id", "id={$dados['id']}", !0);
            ...
            foreach (Metadados::getDicionario($entity) as $i => $meta) {
                ...
                elseif ($meta['format'] === "source_list" && in_array(["valor" => "png", "representacao" => "png"], $meta['allow']['options']))
                    $user['imagem'] = !empty($dados[$meta['column']]) ? $dados[$meta['column']] : null;
            }

            $read = new Read();
            if ($action === "create" && !empty($user['password']) && !empty($user['nome'])) {
                $read->exeRead("usuarios", "WHERE nome = '{$user['nome']}' && password = :p", "p={$user['password']}", !0);
                if (!$read->getResult()) {
                ...
                if ($sql->getResult()) {
                    ...
                    $read->exeRead("usuarios", "WHERE nome = '{$newData['nome']}' && password = :p && id !=:id", "p={$newData['password']}id={$dados['usuarios_id']}", !0);
                    if (!$read->getResult()) {
                        $up = new Update();
